...
# 20240401 - Improvments for ES-de from Pixelbo
#
...
print_help() {
cat << EOF 
create-collection [ -c collection_name ] [ -g genre ] search_terms 
...
Examples:
  "create-collection zelda" creates a custom collection
  ...
  Invoke "create-collection -c basketball 'nba '" if you want a list 
  of NBA games named "custom-basketbell.cfg", without 'pinball' games in.

  "create-collection -c plateformers -g Plateform" creates a 
  collection of all plarform games (into custom-plateformers.cfg).

  Finally, "create-collection -c all" creates a custom-all.cfg 
  ...
  you can start with "create-collection -s nes zelda" and then
  add gb games with "create-collection -s gb zelda".
...
parse_filenames() {
	...
	find ${rompath}/${systempath} -maxdepth ${depth} -type f | egrep -i "${rompath}.*${pattern}.*${extensions}" > ${tmpfile}
...
parse_gamelists() {
	...
	for system in ${systemslist}; do
		romdir=${rompath}/$(basename ${system})
		xmltmp_notclean=${system}/gamelist.xml
		[ -f ${xmltmp_notclean} ] || continue
		xmltmp=/tmp/xml_cleaned_${fn}.xml
		touch $xmltmp
		#Adding a wrapper to ensure that it is valid
		echo "<wrapper>" >> $xmltmp
		echo $(sed "s/<?xml version=\"1.0\"?>//g" ${xmltmp_notclean}) >> $xmltmp
		echo "</wrapper>" >> $xmltmp

		[ -f ${xmltmp} ] || continue
		if ! [[ -z $lowerpattern ]]; then
			if ! [[ -z $genre ]]; then
				xmllint --xpath "/wrapper/gameList/game[contains(name, \"${lowerpattern}\") and contains(genre, \"${genre}\")]/path/text()" ${xmltmp} 2>/dev/null | sed 's:^\./::' > ${tmpfile}
			else
				xmllint --xpath "/wrapper/gameList/game[contains(name, \"${lowerpattern}\")]/path/text()" ${xmltmp} 2>/dev/null | sed 's:^\./::' > ${tmpfile}
			fi
		else
			xmllint --xpath "/wrapper/gameList/game[contains(genre, \"${genre}\")]/path/text()" ${xmltmp} 2>/dev/null | sed 's:^\./::' > ${tmpfile}
		...
		while IFS= read -r game; do 
			# remove [...] in the filename to avoid regex errors
			cleangame=$(echo $game | sed "s:\[.*::")
			if ! fgrep -e "${romdir}/${game}" ${filecollection} >/dev/null 2>/dev/null; then
				[[ -f ${romdir}/${game} ]]&& echo ${romdir}/${game} >> ${filecollection} && new_found=$((new_found+1))
			fi
		done < ${tmpfile}
		[[ -f $xmltmp ]] && rm ${xmltmp}
	done
...
[[  ! -e $filecollection ]] && touch $filecollection
# Let's go!
